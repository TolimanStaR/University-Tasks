...
function Warning () {
	echo -e "\033[93m"
	printf 'К вашему вниманию, Сэр, вы используете небезопасный режим интерпретации. Рзультат  может отличаться от ожидаемого.\n'
	echo -e "\033[00m"
}
...
function advanced_mode () { # Распознает любое корректное выражение, написанное хоть на двух языках и третьей рукой
	...
	if [[ "$strict" = "-abstract" ]]; then
		for (( stat = 0; stat < $(($expression_length - 1)); stat++ )); do
			...
			if [[ "$type" -eq 4 ]]; then
				...
				if [[ "$condition" -eq 1 ]]; then
					...
					if [[ "$res_statement" = "+" || "$res_statement" = "-" ]]; then
						res_statement=${neutral_elems[0]}$res_statement
					else
						res_statement=${neutral_elems[1]}$res_statement
					fi	
				fi
			fi
		...
		for (( stat = 0; stat < $(($expression_length - 1)); stat++ )); do
			...
			if [[ "$type" -eq 4 ]]; then
				...
				if [[ "$condition" -eq 1 ]]; then
					...
					# let condition=$((condition^1))
					# if [[ "${adv_expression[$stat]}" = "+" || "${adv_expression[$stat]}" = "-" ]]; then
					# 	res_statement=0${adv_expression[$stat]}

					# else
					# 	res_statement=1${adv_expression[$stat]}
					# fi
					Exception	
				fi
			fi
...
function main ()
{
	...
	if [[ "$1" = "-a" ]]; then
		...
		advanced_mode "${expression[@]}" $strict
	else
